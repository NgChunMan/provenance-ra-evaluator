...
the annotation passes through unchanged (identity copy).  If it does
not hold, the tuple is excluded from the output (absent from the
support).

The semiring operation for selection is identity — annotations are
neither added nor multiplied, only copied for qualifying tuples.
Non-matching tuples are simply skipped.
...
def selection(
    ...
    Args:
        ...
        predicate (Callable[[Dict[str, Any]], bool]): A function that takes a
            row as a column→value dict and returns ``True`` if the row should
            be included in the result.

    Returns:
        KRelation: New relation containing only the tuples for which the
        predicate holds, with annotations unchanged.
